...
>(
  (
    ...
    const isMobileHookResult = useIsMobileHook(); 
    ...
    // Initialize isMobile as false for SSR and first client render, update via useEffect
    const [isClientDeterminedMobile, setIsClientDeterminedMobile] = React.useState(false);
    
    React.useEffect(() => {
        if (isMobileHookResult !== undefined) {
            setIsClientDeterminedMobile(isMobileHookResult);
        }
    }, [isMobileHookResult]);
...
>(({ className, onClick, title = "Ouvrir/Fermer le menu latéral", ...props }, ref) => {
